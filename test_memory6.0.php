...
$st = new tosca_service_template();
	...
	$tt = new tosca_topology_template();
		...
		for ($i = 1; $i <= 9000; $i++) {
			$node = 'NODE'.$i;
			$tt->node_templates(array($node => $VDU1->get()));
		}
...
mem_usage('completata costruzione del template');


echo $st->yaml('prova.yml');

// $stp = new tosca_service_template('prova.yml');
$stp = new tosca_service_template($st->get());

mem_usage('completato parsing del template');
